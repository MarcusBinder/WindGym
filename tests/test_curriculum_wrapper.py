...
class DummyEnv(gym.Env):
    ...
    def __init__(self):
        ...
        self.ws, self.wd, self.ti = 10., 270., 0.05

    def reset(self, **_):
        return np.zeros(1, dtype=np.float32), {"yaw angles agent": np.array([10., -5.])}

    def step(self, action):                                        # noqa: D401
        return (np.zeros(1, dtype=np.float32), 10.0, False, False,
                {"yaw angles agent": np.array([10., -5.])})


class DummyPyWakeAgent:
    """Just delivers a fixed ‘optimal’ yaw vector; no optimisation."""
    def __init__(self, **_):
        self.optimized_yaws = np.array([5., -5.])
    def update_wind(self, *_): pass
...
def wrapper(patched_cw):
    ...
    weight_fn = lambda _step: 0.3
    return patched_cw.CurriculumWrapper(DummyEnv(), n_envs=2,
...
MAX_L2  = np.sqrt(2) * 90.   # 2 turbines, yaw-range 90°


def expected_similarity(kind: str,
                        huber_kappa=1., exp_alpha=1.) -> float:
    if kind == "l2":             return -L2
    if kind == "l1":             return -np.mean(np.abs(DIFF))
    if kind == "mse":            return -np.mean(DIFF**2)
    if kind == "normalized_l2":  return 1 - L2 / MAX_L2
    if kind == "exponential":    return float(np.exp(-exp_alpha * L2))
    if kind == "cosine":
    ...
    if kind == "huber":
        ...
        loss  = np.where(quad, 0.5*DIFF**2,
                         huber_kappa*(abs_d - 0.5*huber_kappa))
...
#  Tests                                                                      #
# --------------------------------------------------------------------------- #
@pytest.mark.parametrize("similarity_type",
                         ["l2", "l1", "mse", "normalized_l2",
                          "exponential", "cosine", "huber"])
...
def test_yaw_check_goal(wrapper):
    ...
    assert np.allclose(scaled, [0., 22.5], atol=1e-6)
