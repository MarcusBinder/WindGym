...
def get_base_yaml_dict(nx=2, ny=1, history_length=5, window_length=2, history_n=1):
    ...
    return {
        ...
        "ActionMethod": "yaw",
        "Track_power": False,
        "farm": {"yaw_min": -30, "yaw_max": 30, "xDist": 5, "yDist": 3, "nx": nx, "ny": ny},
        ...
        "ws_mes": {"ws_current": True, "ws_rolling_mean": True, "ws_history_N": history_n, "ws_history_length": history_length, "ws_window_length": window_length},
        "wd_mes": {"wd_current": True, "wd_rolling_mean": True, "wd_history_N": history_n, "wd_history_length": history_length, "wd_window_length": window_length},
        "yaw_mes": {"yaw_current": True, "yaw_rolling_mean": True, "yaw_history_N": history_n, "yaw_history_length": history_length, "yaw_window_length": window_length},
        "power_mes": {"power_current": True, "power_rolling_mean": True, "power_history_N": history_n, "power_history_length": history_length, "power_window_length": window_length},
    }
...
def temp_yaml_filepath_factory():
    ...
    def _create_temp_yaml(config_dict, name_suffix=""):
        ...
        tf = tempfile.NamedTemporaryFile(mode="w", delete=False, suffix=f"_{name_suffix}.yaml", encoding='utf-8')
...
def calculate_expected_obs_dim(config_dict, n_turbines):
    ...
    if mes_level.get("turb_power", False):
        one_turb_mes_obs_actual += power_mes_cfg.get("power_current", False) + \
                                   (power_mes_cfg.get("power_rolling_mean", False) * power_mes_cfg.get("power_history_N", 0))
...
class TestSpecificFeatures:
    @pytest.mark.parametrize("fill_window_config_val, expected_steps_on_reset_factor_str", [
        (True, "hist_max"), 
        (3, "val_3"), 
        (False, "val_1"),
        (10, "hist_max_capped") 
    ])
    def test_fill_window_logic(self, temp_yaml_filepath_factory, fill_window_config_val, expected_steps_on_reset_factor_str, mock_mann_methods):
        base_hist_len = 5
        ...
        yaml_filepath = temp_yaml_filepath_factory(config_dict, f"fill_window_{str(fill_window_config_val).lower()}")
        
        hist_max_calculated = base_hist_len 
        
        env = WindFarmEnv(turbine=V80(), yaml_path=yaml_filepath, 
                          fill_window=fill_window_config_val, reset_init=True, seed=42, turbtype="None",
                          Baseline_comp=False) 
        ...
        assert env.steps_on_reset == expected_steps, f"Calculated hist_max: {hist_max_calculated}"
        check_env(env, skip_render_check=True) 
        env.close()

    @pytest.mark.parametrize("render_mode_val", [
        None,
        "human",
    ])
    def test_render_mode_behavior(self, temp_yaml_filepath_factory, render_mode_val, mock_mann_methods):
        """
        Tests supported render modes. 'rgb_array' is removed as requested.
        """
        config_dict = get_base_yaml_dict()
        yaml_filepath = temp_yaml_filepath_factory(config_dict, f"render_{render_mode_val}")
        
        env = WindFarmEnv(turbine=V80(), yaml_path=yaml_filepath, 
                          render_mode=render_mode_val, reset_init=True, seed=42, turbtype="None")
        ...
        try:
            ...
            if "TclError" in str(e) and render_mode_val == "human": # pragma: no cover
                pytest.skip("Skipping human render mode test in headless environment due to TclError.")
            else: # pragma: no cover
                raise e
        finally:
            env.close()

    @pytest.mark.parametrize("turb_wd_enabled, farm_wd_enabled, wd_current, wd_rolling, wd_hist_n", [
    ...
    def test_wind_direction_features_in_observations(
        ...
        config_dict = get_base_yaml_dict(nx=2, ny=1, history_length=5, window_length=2, history_n=max(1,wd_hist_n)) 
        ...
        config_dict["wd_mes"] = {
            "wd_current": wd_current, "wd_rolling_mean": wd_rolling, "wd_history_N": wd_hist_n, 
            "wd_history_length": 5, "wd_window_length": 2
        }
        ...
        config_dict["ws_mes"]["ws_current"] = False; config_dict["ws_mes"]["ws_rolling_mean"] = False
        config_dict["power_mes"]["power_current"] = False; config_dict["power_mes"]["power_rolling_mean"] = False
        config_dict["yaw_mes"] = {"yaw_current": True, "yaw_rolling_mean": False, "yaw_history_N": 0, "yaw_history_length": 5, "yaw_window_length": 2}


        yaml_filepath = temp_yaml_filepath_factory(config_dict, f"obs_wd_{turb_wd_enabled}_{farm_wd_enabled}_{wd_current}_{wd_rolling}_{wd_hist_n}")
        env = WindFarmEnv(turbine=V80(), yaml_path=yaml_filepath, reset_init=True, seed=42, turbtype="None", Baseline_comp=False)
    ...
    @pytest.mark.parametrize("mes_type_key, current_flag, rolling_flag, history_n_val", [
        ("ws_mes", True, False, 0), 
        ("ws_mes", False, True, 3), 
        ("ws_mes", True, True, 1),  
    ...
    def test_specific_measurement_configurations(
        ...
        n_turb_nx = 2
        config_dict = get_base_yaml_dict(nx=n_turb_nx, ny=1, history_length=10, window_length=3, history_n=max(1, history_n_val))

        config_dict["power_def"]["Power_reward"] = "None"
        config_dict["act_pen"]["action_penalty"] = 0.0

        config_dict["mes_level"] = {
            "turb_ws": False, "turb_wd": False, "turb_TI": False, "turb_power": False,
        ...
        config_dict["yaw_mes"] = {"yaw_current": True, "yaw_rolling_mean": False, "yaw_history_N": 0, "yaw_history_length": 10, "yaw_window_length": 3}

        yaml_filepath = temp_yaml_filepath_factory(config_dict, f"obs_cfg_{mes_type_key}_{current_flag}_{rolling_flag}_{history_n_val}")
        env = WindFarmEnv(turbine=V80(), yaml_path=yaml_filepath, reset_init=True, seed=42, turbtype="None", Baseline_comp=False)
    ...
    def test_track_power_not_implemented(self, temp_yaml_filepath_factory, mock_mann_methods):
        ...
        with pytest.raises(NotImplementedError, match="The Track_power is not implemented yet"):
            WindFarmEnv(turbine=V80(), yaml_path=yaml_filepath, reset_init=True, seed=42, turbtype="None")
