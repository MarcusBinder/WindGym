...
def test_step_determinism_basic(deterministic_env_config_path):
    ...
    try:
        ...
        # --- Assertions ---
        assert data_equivalence(next_obs1, next_obs2), \
            f"Observations are not deterministic.\nObs1: {next_obs1}\nObs2: {next_obs2}"

        assert np.isclose(reward1, reward2, atol=1e-8), \
        ...
        assert np.isclose(info_step1["Power agent"], info_step2["Power agent"], atol=1e-8), \
             f"Agent Power in info not deterministic. P1: {info_step1['Power agent']}, P2: {info_step2['Power agent']}"
        if env.Baseline_comp:
            assert np.isclose(info_step1["Power baseline"], info_step2["Power baseline"], atol=1e-8), \
...
def test_step_determinism_with_noise(deterministic_env_config_path): # Still uses the base YAML initially
    """
