...
class TestColiseumAdvanced:
    ...
    def test_create_env_factory_with_site(self, temp_yaml_file):
        ...
        mock_kwargs = {'x_pos': [0], 'y_pos': [0], 'turbine': V80(), 'yaml_path': temp_yaml_file}
    ...
    def test_pywake_agent_update_wind_is_called(self, temp_yaml_file):
        ...
        with patch.object(pywake_agent, 'update_wind', wraps=pywake_agent.update_wind) as mock_update_method:
            # 4. Run a single-point grid evaluation
            ws_test, wd_test, ti_test = 10, 270, 0.08
            
            coliseum.run_wind_grid_evaluation(
                ws_min=ws_test, ws_max=ws_test, ws_step=1,
                wd_min=wd_test, wd_max=wd_test, wd_step=1,
                ti_min=ti_test, ti_max=ti_test, ti_points=1
            )
...
class TestColiseumIntegration:
    ...
    def test_create_env_factory_with_site(self, temp_yaml_file_integration):
        """Covers the static helper method for creating a site-based factory."""
        mock_site = Hornsrev1Site()
        mock_kwargs = {'x_pos': [0], 'y_pos': [0], 'turbine': V80(), 'yaml_path': temp_yaml_file_integration}
    ...
    def test_pywake_agent_in_time_series_evaluation(self, temp_yaml_file_integration):
        ...
        env_factory = lambda: FarmEval(
            ...
            yaml_path=temp_yaml_file_integration, turbtype="None",
            reset_init=True, finite_episode=True
        )
        coliseum = Coliseum(env_factory, agents={"PyWake": pywake_agent})
    ...
    def test_pywake_agent_in_grid_evaluation(self, temp_yaml_file_integration):
        ...
        class SpyPyWakeAgent(PyWakeAgent):
            ...
            def update_wind(self, wind_speed, wind_direction, TI):
                self.update_calls.append({'ws': wind_speed, 'wd': wind_direction, 'ti': TI})
        ...
        coliseum.run_wind_grid_evaluation(
            ws_min=min(ws_grid), ws_max=max(ws_grid), ws_step=2,
            wd_min=min(wd_grid), wd_max=max(wd_grid), wd_step=10,
            ti_min=ti_val, ti_max=ti_val, ti_points=1
        )
