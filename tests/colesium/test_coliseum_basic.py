...
@pytest.fixture
def coliseum_instance_timeseries(temp_yaml_file_for_coliseum, coliseum_agents_timeseries):
    """
...
def test_run_wind_grid_evaluation_basic(coliseum_instance, coliseum_agents):
    ...
    dataset = coliseum_instance.run_wind_grid_evaluation(
        wd_min=wd_min, wd_max=wd_max, wd_step=wd_step,
        ws_min=ws_min, ws_max=ws_max, ws_step=ws_step,
        ti_points=ti_points
    )
...
def test_run_wind_grid_evaluation_single_point(coliseum_instance):
    ...
    """
    dataset = coliseum_instance.run_wind_grid_evaluation(
        wd_min=270, wd_max=270, wd_step=5,
        ws_min=10, ws_max=10, ws_step=2,
        ti_points=1
    )
...
def test_plot_wind_grid_results_runs_without_error(mock_savefig, coliseum_instance):
    ...
    dataset = coliseum_instance.run_wind_grid_evaluation(
        wd_min=270, wd_max=275, wd_step=5,
        ws_min=8, ws_max=10, ws_step=2,
        ti_points=1
    )
    ...
    mock_savefig.assert_called_once_with("test_grid_plot.png", dpi=150, bbox_inches='tight')


# --- Time Series Evaluation Tests ---

def test_coliseum_run_time_series_evaluation(coliseum_instance_timeseries, coliseum_agents_timeseries):
    """
    ...
    assert len(ts_results) == len(coliseum_agents_timeseries), "Number of agents in time_series_results should match input."

    for agent_name, results_list in ts_results.items():
        assert agent_name in coliseum_agents_timeseries, f"Unexpected agent '{agent_name}' found in results."
...
@patch('matplotlib.pyplot.savefig')
def test_coliseum_plot_time_series_runs_without_error(mock_savefig, coliseum_instance_timeseries):
    """
    ...
    coliseum_instance_timeseries.run_time_series_evaluation(num_episodes=1, save_detailed_history=True)
