...
class TestColiseumEdgeCases:
    ...
    def test_init_raises_error_for_mismatched_labels(self, fast_farm_eval_factory):
        ...
        with pytest.raises(ValueError, match="Number of agent_labels must match number of agents"):
            Coliseum(fast_farm_eval_factory, agents=agents_list, agent_labels=agent_labels)
    ...
    def test_init_with_custom_agent_labels(self, fast_farm_eval_factory):
        ...
        coliseum = Coliseum(fast_farm_eval_factory, agents=agents_list, agent_labels=custom_labels)
    ...
    def test_init_raises_error_for_empty_agents(self, fast_farm_eval_factory, empty_agents):
        """
    ...
    def test_update_wind_warning_in_timeseries_eval(self, capsys):
        ...
        class AgentWithUpdateWind:
            def predict(self, obs, deterministic): return np.array([0]), None
            def update_wind(self, wind_speed, wind_direction, TI): pass

        # FIX: Add 'close' and 'sample_site' to the spec list for the mock.
        spec_list = ['ws', 'wd', 'reset', 'step', 'n_passthrough', 'close', 'sample_site']
        ...
        mock_env.step.return_value = (np.zeros(10), 0, True, False, {}) # Ensure episode terminates
        ...
        captured = capsys.readouterr()
        assert "Warning: Agent has 'update_wind' method, but env is missing" in captured.out
    ...
    def test_update_wind_warning_in_grid_eval(self, capsys):
        ...
        spec_list = [
            'ws', 'wd', 'reset', 'step', 'set_wind_vals', 'n_passthrough', 
            'close', 'wd_min', 'wd_max', 'ws_min', 'ws_max', 'TI_min', 'TI_max'
        ]
        mock_env = MagicMock(spec=spec_list)
    ...
    def test_plot_time_series_prints_warning_if_no_data(self, coliseum_instance, capsys):
        """
        ...
        captured = capsys.readouterr()
        expected_warning = "No time series data available. Run time series evaluation first."
