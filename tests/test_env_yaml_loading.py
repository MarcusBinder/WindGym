...
def get_base_yaml_dict():
    return {
        "yaw_init": "Zeros", "noise": "None", "BaseController": "Local", "ActionMethod": "yaw", "Track_power": False,
...
@pytest.mark.parametrize("setting, value", [
    ("yaw_init", "Random"), ("yaw_init", "Zeros"),
    ("noise", "Normal"), ("noise", "None"),
    ("BaseController", "Global"), ("BaseController", "Local"),
    ("ActionMethod", "wind"), ("ActionMethod", "yaw"),
])
def test_initial_settings_loading(temp_yaml_file_factory, setting, value):
    ...
    env = WindFarmEnv(turbine=V80(), yaml_path=yaml_filepath, reset_init=False) # reset_init=False to speed up, only check config loading
...
def test_farm_params_loading(temp_yaml_file_factory):
    config_dict = get_base_yaml_dict()
    config_dict["farm"] = {"yaw_min": -25, "yaw_max": 25, "xDist": 4, "yDist": 4, "nx": 3, "ny": 2}
    yaml_content = yaml.dump(config_dict)
...
def test_wind_params_loading(temp_yaml_file_factory):
    config_dict = get_base_yaml_dict()
    config_dict["wind"] = {"ws_min": 7, "ws_max": 12, "TI_min": 0.03, "TI_max": 0.12, "wd_min": 200, "wd_max": 300}
    yaml_content = yaml.dump(config_dict)
...
@pytest.mark.parametrize("mes_config_key, mes_config_value, expected_attr_dict", [
...
def test_measurement_level_loading(temp_yaml_file_factory, mes_config_key, mes_config_value, expected_attr_dict):
    config_dict = get_base_yaml_dict()
    config_dict["mes_level"][mes_config_key] = mes_config_value
    yaml_content = yaml.dump(config_dict)
    yaml_filepath = temp_yaml_file_factory(yaml_content, f"mes_level_{mes_config_key}_{mes_config_value}")
    ...
    for attr_path, expected_val in expected_attr_dict.items():
        ...
        assert getattr(obj, parts[-1]) == expected_val, f"Attribute {attr_path} mismatch"
    env.close()

# --- Tests for Specific Measurement Settings (e.g., ws_mes) ---
def test_ws_mes_settings_loading(temp_yaml_file_factory):
    config_dict = get_base_yaml_dict()
    config_dict["ws_mes"] = {"ws_current": False, "ws_rolling_mean": True, "ws_history_N": 2, "ws_history_length": 10, "ws_window_length": 3}
    yaml_content = yaml.dump(config_dict)
    yaml_filepath = temp_yaml_file_factory(yaml_content, "ws_mes_custom")

    env = WindFarmEnv(turbine=V80(), yaml_path=yaml_filepath, reset_init=False) # Calls _init_farm_mes
