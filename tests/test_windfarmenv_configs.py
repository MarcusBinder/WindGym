...
@pytest.mark.parametrize(
    ...
    [
        (
            BASE_YAML_CONFIG_STRING_FOR_TESTS.format(
                noise_setting="None",
                include_turbine_wd_yaml="true",
                include_farm_wd_yaml="true",
            ),
            "NoNoise_AllWD",
        ),
        (
            BASE_YAML_CONFIG_STRING_FOR_TESTS.format(
                noise_setting="Normal",
                include_turbine_wd_yaml="true",
                include_farm_wd_yaml="true",
            ),
            "NormalNoise_AllWD",
...
def test_wind_farm_env_configurations(temp_yaml_file_path, description):
    ...
    with open(temp_yaml_file_path, "r") as f:
        ...
        critical_history_length = yaml_config.get("ws_mes", {}).get(
            "ws_history_length", 5
        )
    ...
    ), f"Observation shape mismatch (Obs: {obs.shape}, Space: {env.observation_space.shape}) for {description}"
    assert isinstance(info, dict), "Info should be a dictionary"

    num_raw_features = env._get_num_raw_features()
    expected_obs_len = env.farm_measurements.observed_variables()
    assert (
        num_raw_features <= expected_obs_len
    ), f"Raw features count ({num_raw_features}) mismatch with farm_measurements config ({expected_obs_len}) for {description}"
    assert (
        len(obs) == expected_obs_len
    ), f"Observation length {len(obs)} doesn't match expected {expected_obs_len} for {description}"
    ...
    for i in range(3):  # A few steps for basic functional check
        ...
        assert isinstance(
            obs, np.ndarray
        ), f"Observation should be a numpy array after step {i+1}"
        assert (
            obs.shape == env.observation_space.shape
        ), f"Observation shape mismatch after step {i+1}"
        assert isinstance(reward, float), f"Reward should be a float after step {i+1}"
        assert isinstance(
            terminated, bool
        ), f"Terminated flag should be a bool after step {i+1}"
        assert isinstance(
            truncated, bool
        ), f"Truncated flag should be a bool after step {i+1}"
...
def test_wind_direction_in_observations(temp_yaml_file_path, expected_wd_in_obs_calc):
    ...
    try:
        env = WindFarmEnv(
            turbine=V80(), yaml_path=temp_yaml_file_path, seed=43, turbtype="None"
        )
        _ = env.reset()
        ...
        total_expected_wd_features_in_obs = (
            total_turb_wd_features + num_farm_wd_features
        )

        if expected_wd_in_obs_calc:
            assert (
                total_expected_wd_features_in_obs > 0
            ), f"Wind direction features expected to be present but calculated count is {total_expected_wd_features_in_obs}."
        else:
            assert (
                total_expected_wd_features_in_obs == 0
            ), f"Wind direction features expected to be absent but calculated count is {total_expected_wd_features_in_obs}."
    finally:
        if env:
            env.close()
