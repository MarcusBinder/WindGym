...
def assemble_base_yaml_config_string():
    config = {
        ...
        "ActionMethod": "yaw", "Track_power": False,
        ...
        "ws_mes": {"ws_current": True, "ws_rolling_mean": True, "ws_history_N": 3, "ws_history_length": 10, "ws_window_length": 2},
        "wd_mes": {"wd_current": True, "wd_rolling_mean": True, "wd_history_N": 3, "wd_history_length": 10, "wd_window_length": 2},
        "yaw_mes": {"yaw_current": True, "yaw_rolling_mean": True, "yaw_history_N": 3, "yaw_history_length": 10, "yaw_window_length": 2},
        "power_mes": {"power_current": True, "power_rolling_mean": True, "power_history_N": 3, "power_history_length": 10, "power_window_length": 2},
    }
...
def create_mock_mann_field_instance(monkeypatch):
    ...
    def mock_scale_ti(self_field, TI, U): # Renamed 'self' to 'self_field' to avoid clash
    ...
    mocked_tf.x, mocked_tf.y, mocked_tf.z = [np.arange(N) * d for N, d in zip(mocked_tf.Nxyz, mocked_tf.dxyz)]
...
class TestTurbulenceLoading:
    ...
    def test_turbtype_none(self, temp_yaml_filepath_factory):
        ...
        env = WindFarmEnv(turbine=V80(), yaml_path=yaml_filepath, turbtype="None", reset_init=True, seed=42, Baseline_comp=False)
    ...
    def test_turbtype_mannload_single_file(self, tmp_path, temp_yaml_filepath_factory, monkeypatch):
        yaml_content = assemble_base_yaml_config_string()
        ...
        def mock_from_netcdf(filename):
            ...
            mocked_tf_instance = create_mock_mann_field_instance(monkeypatch) # Pass monkeypatch
        ...
        env = WindFarmEnv(turbine=V80(), yaml_path=yaml_filepath, turbtype="MannLoad", TurbBox=str(tf_file_path), reset_init=True, seed=42)
        ...
        assert isinstance(env.addedTurbulenceModel, SynchronizedAutoScalingIsotropicMannTurbulence)
        assert isinstance(env.site.turbulenceField, MannTurbulenceField)
        check_env(env, skip_render_check=True)
        env.close()

    def test_turbtype_mannload_directory(self, tmp_path, temp_yaml_filepath_factory, monkeypatch):
        yaml_content = assemble_base_yaml_config_string()
        ...
        env = WindFarmEnv(turbine=V80(), yaml_path=yaml_filepath, turbtype="MannLoad", TurbBox=str(turb_dir), reset_init=True, seed=42)
        assert env.turbtype == "MannLoad"
        ...
        assert len(from_netcdf_called_with) == 1, "from_netcdf should be called once for the selected file"
    ...
    def test_turbtype_mannload_directory_empty_fallback(self, tmp_path, temp_yaml_filepath_factory, monkeypatch):
        yaml_content = assemble_base_yaml_config_string()
        ...
        env = WindFarmEnv(
            turbine=V80(), yaml_path=yaml_filepath, turbtype="MannLoad",
            TurbBox=str(empty_turb_dir), reset_init=True, seed=42
        )
    ...
    def test_turbtype_manngenerate(self, temp_yaml_filepath_factory, monkeypatch):
        ...
        env = WindFarmEnv(turbine=V80(), yaml_path=yaml_filepath, turbtype="MannGenerate", reset_init=True, seed=42)
        assert env.turbtype == "MannGenerate"
    ...
    def test_turbtype_random(self, temp_yaml_filepath_factory):
        ...
        env = WindFarmEnv(turbine=V80(), yaml_path=yaml_filepath, turbtype="Random", reset_init=True, seed=42, Baseline_comp=False)
    ...
    def test_turbtype_invalid(self, temp_yaml_filepath_factory):
        ...
        with pytest.raises(ValueError, match="Invalid turbulence type specified"):
            WindFarmEnv(turbine=V80(), yaml_path=yaml_filepath, turbtype="ThisIsAnInvalidTurbulenceType", reset_init=True, seed=42)
    ...
    def test_turbtype_mannfixed(self, temp_yaml_filepath_factory, monkeypatch):
        ...
        env = WindFarmEnv(turbine=V80(), yaml_path=yaml_filepath, turbtype="MannFixed", reset_init=True, seed=123)
