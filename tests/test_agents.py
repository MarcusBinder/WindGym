...
def test_greedy_agent_local_controller(base_example_data_path):
    ...
    """
    yaml_path = base_example_data_path / Path("Env1.yaml") # Using Env1.yaml as in the pattern
    env = FarmEval(
        turbine=V80(),
        yaml_path=yaml_path,
    ...
    assert state is None, "GreedyAgent's predict method should return None for the state."
    assert isinstance(action, np.ndarray), "Action should be a numpy array."
    assert action.shape == env.action_space.shape, f"Action shape mismatch. Expected {env.action_space.shape}, got {action.shape}."
    assert np.all(action >= -1) and np.all(action <= 1), "Action values should be scaled between -1 and 1."
    env.close()
