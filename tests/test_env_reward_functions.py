...
"""
    return "\n".join([
        YAML_HEADER_MINIMAL, YAML_FARM_MINIMAL, YAML_WIND_FIXED,
        YAML_ACTION_PENALTY_ZERO, power_def_yaml,
        YAML_MES_LEVEL_MINIMAL, YAML_MEASUREMENT_SETTINGS_MINIMAL
    ])
...
def run_env_and_get_reward(yaml_content, temp_yaml_file_factory, constructor_overrides=None):
...
def test_power_reward_baseline(temp_yaml_file_factory, mock_turbulence_env_setup):
    ...
    if baseline_power != 0:
        ...
        pytest.skip("Baseline power is zero, reward calculation is undefined or error in env.")
...
def test_power_reward_power_avg(temp_yaml_file_factory, mock_turbulence_env_setup):
    ...
    reward, info, env = run_env_and_get_reward(
        yaml_content, temp_yaml_file_factory, {"Baseline_comp": False} # Not strictly needed but good for isolation
    )

    agent_power_sum = info["Power agent"] # This is sum of turbine powers for current step
    ...
    assert np.isclose(reward, expected_reward, atol=1e-4), \
...
def test_power_reward_none(temp_yaml_file_factory, mock_turbulence_env_setup):
    ...
    expected_reward = 0.0 
    assert np.isclose(reward, expected_reward, atol=1e-8), \
...
def test_power_reward_power_diff(temp_yaml_file_factory, mock_turbulence_env_setup):
    ...
    yaml_content = assemble_reward_test_yaml(power_reward_type="Power_diff", power_avg=power_avg_val)
    
    yaml_filepath = temp_yaml_file_factory(yaml_content, "reward_power_diff")
    env = WindFarmEnv(
        turbine=V80(), yaml_path=yaml_filepath, seed=123,
        dt_sim=1, dt_env=10, turbtype="None",
    ...
    yaml_content_invalid = assemble_reward_test_yaml(power_reward_type="Power_diff", power_avg=10)
    yaml_filepath_invalid = temp_yaml_file_factory(yaml_content_invalid, "power_diff_invalid")
    with pytest.raises(ValueError, match="The Power_avg must be larger then 40 for the Power_diff reward"):
    ...
    env.close()
    if os.path.exists(yaml_filepath): os.remove(yaml_filepath)
    if os.path.exists(yaml_filepath_invalid): os.remove(yaml_filepath_invalid)

# --- Test for Action Penalty Type ---
@pytest.mark.parametrize("action_type, penalty_value, initial_yaw_offset, step_action, expected_penalty_factor", [
    ("Change", 0.1, 0.0, np.array([0.5, 0.5]), 0.5 * 1.0), # action * yaw_step = 0.5 change
    ("Change", 0.1, 10.0, np.array([-0.5, -0.5]), 0.5 * 1.0),# action * yaw_step = -0.5 change
    ("Total", 0.1, 0.0, np.array([1.0, 1.0]), (1.0 * 1.0) / 30.0), # new yaw = 1.0. Max_yaw is 30 (from YAML)
    ("Total", 0.1, 10.0, np.array([-1.0, -1.0]), (10.0 - 1.0) / 30.0),# new yaw = 9.0
])
def test_action_penalty_logic(temp_yaml_file_factory, action_type, penalty_value, initial_yaw_offset, step_action, expected_penalty_factor):
    power_def_yaml = f"""
power_def:
...
"""
    yaml_content = "\n".join([
        YAML_HEADER_MINIMAL, YAML_FARM_MINIMAL, YAML_WIND_FIXED,
        action_pen_yaml, power_def_yaml,
        YAML_MES_LEVEL_MINIMAL, YAML_MEASUREMENT_SETTINGS_MINIMAL
    ])
    ...
    env = WindFarmEnv(
        turbine=V80(), yaml_path=yaml_filepath, seed=123,
        dt_sim=1, dt_env=10, yaw_step=1.0, # yaw_step = 1.0 for direct action mapping
        turbtype="None", fill_window=1, reset_init=True
    )
    
    # Manually set initial yaw for "Change" penalty predictability
    obs, info = env.reset(seed=123)
    env.fs.windTurbines.yaw = np.array([initial_yaw_offset] * env.n_turb, dtype=np.float32)
    if env.Baseline_comp: # Should be false if Power_reward is "None"
         env.fs_baseline.windTurbines.yaw = np.array([initial_yaw_offset] * env.n_turb, dtype=np.float32)
    ...
    assert np.isclose(actual_penalty_component, expected_calculated_penalty, atol=1e-5), \
