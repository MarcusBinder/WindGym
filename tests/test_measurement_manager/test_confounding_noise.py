...
def test_episodic_bias_confounds_yaw_with_wd():
    ...
    specs = [
        MeasurementSpec(name="turb_0/wd_current", measurement_type=MeasurementType.WIND_DIRECTION,
                        index_range=(0, 1), min_val=260, max_val=280, turbine_id=0, is_circular=True),
        MeasurementSpec(name="turb_0/yaw_current", measurement_type=MeasurementType.YAW_ANGLE,
                        index_range=(1, 2), min_val=-30, max_val=30, turbine_id=0),
    ]
    ...
    yaw_spec = specs[1]
    expected_scaled_yaw_delta = (expected_final_yaw_bias_physical * 2.0) / (yaw_spec.max_val - yaw_spec.min_val)
...
def test_white_noise_confounds_yaw_with_wd():
    ...
    # 3. ASSERT
    expected_final_yaw_noise_physical = YAW_NOISE_PHYSICAL - WD_NOISE_PHYSICAL # -1.0 - 5.0 = -6.0
    yaw_spec = specs[1]
    expected_scaled_yaw_delta = (expected_final_yaw_noise_physical * 2.0) / (yaw_spec.max_val - yaw_spec.min_val)
...
def test_agent_perception_is_confounded(smoke_test_env):
    ...
    noisy_env = NoisyWindFarmEnv(
        base_env_class=lambda **kwargs: smoke_test_env, 
    ...
    true_yaw_physical = manager._unscale_value(np.array([true_yaw_offset_scaled]), yaw_spec.min_val, yaw_spec.max_val).item()
