...
class WindGymCustomMonitor(BaseCallback):
    ...
    def _on_step(self) -> bool:
        ...
        for env_idx in range(len(infos)):
            info = infos[env_idx]
            log_data = {
                f"custom_env_{env_idx}/global_wind_speed": info.get("Wind speed Global"),
                f"custom_env_{env_idx}/global_wind_dir": info.get("Wind direction Global"),
            ...
            if "Power baseline" in info:
                log_data[f"custom_env_{env_idx}/total_baseline_power"] = info.get("Power baseline")
                if info.get("Power baseline", 0) != 0:
                    log_data[f"custom_env_{env_idx}/power_gain_vs_baseline"] = \
                        ...
                        if info.get("Power baseline") else 0
            
            num_turbines = len(info.get("yaw angles agent", []))
            for i in range(num_turbines):
                log_data[f"custom_env_{env_idx}/turbine_{i}/yaw_angle"] = info.get("yaw angles agent", [np.nan]*num_turbines)[i]
                log_data[f"custom_env_{env_idx}/turbine_{i}/power"] = info.get("Power pr turbine agent", [np.nan]*num_turbines)[i]
                log_data[f"custom_env_{env_idx}/turbine_{i}/local_wind_speed"] = info.get("Wind speed at turbines", [np.nan]*num_turbines)[i]
                if "Wind direction at turbines" in info: # Only log if WD is active
                    log_data[f"custom_env_{env_idx}/turbine_{i}/local_wind_dir"] = info.get("Wind direction at turbines", [np.nan]*num_turbines)[i]
...
def make_env(temp_yaml_path, turbine_obj, env_init_params, seed=0):
    def _init():
        env = WindFarmEnv(
            ...
            TurbBox=env_init_params.get("TurbBox", "Hipersim_mann_l5.0_ae1.0000_g0.0_h0_128x128x128_3.000x3.00x3.00_s0001.nc"),
...
def train_agent(args):
    ...
    include_turbine_wd_yaml_str = "false" if args.disable_turbine_wd else "true" # YAML expects lowercase true/false
    ...
    if args.noise_level == "Normal":
        print("INFO: WindGym's built-in 'Normal' noise will be applied to observations.")
    ...
    run = wandb.init(
        ...
        config={**vars(args), "yaml_config_string": current_yaml_config_string}, # Log all args and YAML
        name=run_name,
    ...
    if not os.path.isfile(turbbox_path_train) and args.turbtype == "MannLoad":
        ...
        if os.path.isfile(potential_path_cwd):
            ...
            print(f"ERROR: Turbulence box file for MannLoad not found at '{turbbox_path_train}'.")
            exit()
    ...
    try:
        with tempfile.NamedTemporaryFile(mode="w", delete=False, suffix=".yaml") as tmp_yaml_file:
        ...
        vec_env = SubprocVecEnv(
            [make_env(temp_yaml_filepath, turbine, env_init_params, seed=args.seed + i) for i in range(args.n_envs)]
        )
        ...
        eval_env_instance = make_env(temp_yaml_filepath, turbine, env_init_params, seed=args.seed + 1000)()
        ...
        all_callbacks = [wandb_sb3_callback, checkpoint_callback, eval_callback, custom_monitor_callback]

        policy_kwargs = dict(net_arch=dict(pi=[int(x) for x in args.net_arch_pi.split(',')], 
        ...
        print(f"Model and logs will be saved in subdirectories of: {args.models_base_dir} and {args.tensorboard_base_dir}")
        model.learn(
        ...
        if 'eval_env_instance' in locals(): eval_env_instance.close()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--noise_level", type=str, choices=["None", "Normal"], default="None", help="Observation noise level for YAML.")
    parser.add_argument("--disable_turbine_wd", action="store_true", help="Disable turbine wind direction measurements.")
    parser.add_argument("--disable_farm_wd", action="store_true", help="Disable farm wind direction measurements.")
